...
import { resolveShareLink, type VerificationResult } from '@/lib/api'
...
const verdictConfig: Record<Verdict, { label: string; sub: string; color: string; bg: string; Icon: typeof ShieldCheck }> = {
  ...
  revoked:   { label: 'АННУЛИРОВАН',   sub: 'Запись отозвана оператором ВУЗа',      color: 'text-red-400',          bg: 'bg-red-400/5 border-red-400/15',      Icon: ShieldX     },
...
export function PublicSharePage() {
  ...
  useEffect(() => {
    if (!shareToken) {
      setError('Share-токен не указан')
      setLoading(false)
      return
    }
    let cancelled = false
    setLoading(true)
    void resolveShareLink(shareToken)
  ...
  return (
    <div className="mx-auto flex min-h-[70vh] max-w-2xl flex-col justify-center gap-10">
      ...
      >
        <p className="font-mono text-[9px] tracking-[0.2em] text-muted-foreground/40 uppercase">— Публичная ссылка · Для работодателя</p>
        <p className="mt-2 text-sm text-muted-foreground/60">
          Студент предоставил вам временную ссылку для подтверждения диплома.
        </p>
      </motion.div>
      ...
      >
        <Button asChild variant="ghost" size="sm">
          <Link to="/student">← Вернуться в кабинет</Link>
        </Button>
      </motion.div>
